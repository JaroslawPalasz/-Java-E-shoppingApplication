...
public class Eshopping {
     ...
     */
    public Eshopping() throws SQLException, ClassNotFoundException {
        ...
        ResultSet productsResultSet = DatabaseProvider.query("select * from products");
        while(productsResultSet.next()){
            getCategoryList(productsResultSet.getString("category")).add(new Product(
            productsResultSet.getString("category"), productsResultSet.getString("name"), 
                    productsResultSet.getDouble("price"), productsResultSet.getInt("amount"), 
            productsResultSet.getString("description")));
        }
        ResultSet clientsResultSet = DatabaseProvider.query("select * from clients");
        while(clientsResultSet.next()){
            clientMap.put(clientsResultSet.getString("username"), new Client(
            clientsResultSet.getString("username"), clientsResultSet.getString("password"), 
            new Basket(), clientsResultSet.getDouble("money")));
        }
        ...
        category1List.add(new Product("Category1", "c1product13", 1.0, 3, "description of product13"));
        category1List.add(new Product("Category1", "c1product14", 1.0, 3, "description of product14"));
        category1List.add(new Product("Category1", "c1product15", 1.0, 3, "description of product15"));
        category1List.add(new Product("Category1", "c1product16", 1.0, 3, "description of product16"));
     ...
     */
    public void addToBasket(String client, Product product) throws MyException, SQLException, ClassNotFoundException {
        if(clientMap.get(client)==null){
            ...
            if(found==0){
                ...
                DatabaseProvider.update("insert into basket (id, name, category, price, amount, description) values ('"+client+"', '"+product.getName()+"', '"+product.getCategory()+"', "+product.getPrice()+", "+1+", '"+product.getDescription()+"')");
            }
        }
    }
     ...
     */
    public ArrayList<String> convertProductToStringList(ArrayList<Product> productList, ArrayList<String> productNameList){
